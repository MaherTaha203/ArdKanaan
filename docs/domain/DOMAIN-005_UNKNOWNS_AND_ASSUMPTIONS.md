...
| UNK-002 | **The form of a revenue distribution policy.** Only one example exists (1000 → 700/300). Is a policy always a percentage split? Can it be fixed amounts per student, tiered, or per-receipt overrides? Are policies defined per program only, or reusable across programs? How are amounts that don't divide evenly rounded, and who gets the rounding difference? | The split calculation is the heart of the system (F-07). | DR-003, DR-005, WF-03 |
...
| UNK-020 | **Balance composition and accrual timing.** Does a teacher share become "owed" the moment the receipt is recorded? Does the "center balance" mean the center's own share only, or all cash held (including not-yet-paid teacher shares)? | Determines the exact meaning of the two most-read numbers in the system. | DR-009, DR-010, WF-04, WF-10 |
...
3. Next available IDs: **UNK-024**, **ASM-004**.
