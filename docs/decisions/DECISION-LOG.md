...
| [ADR-0075](ADR-0075-SUPERSEDE-BPLUS.md) | Retain B+ Structure, Adopt Current Visual Design — retires the historical B+ visual treatment; the current Owner-designed light visual baseline is the authoritative presentation direction; suitable B+ structure/product decisions remain usable; supersedes ADR-0074 only where it conflicts with the current visual design; changes no frozen constitution, financial rule, phase, or freeze gate; Owner-Decision ADR *(register row backfilled by ADR-0076 — pre-existing gap: the file existed but was unregistered)* | 5 | ACCEPTED | ADR-0074 (visual direction only) |
| [ADR-0076](ADR-0076-student-archive-lifecycle-authorization.md) | Student Archive Lifecycle Authorization — authorizes an independent student-level `archived` state (`active`/`completed`/`archived`) with dedicated `archive_student`/`unarchive_student` RPCs, active-course eligibility, owner-only authorization, and zero financial impact; records an exception to the frozen **DB-014** (DAT-002) for archiving only as a **tracked divergence** reconciled at Documentation Freeze; `completed` kept distinct; financial firewall absolute; opens/advances no phase; Owner-Decision ADR (GOV-013 Panel not invoked) | 5 | ACCEPTED | — |
| [ADR-0077](ADR-0077-student-anchored-financial-obligations.md) | Student-Anchored Financial Obligations — authorizes fee obligations independent of course enrollment (`course_id`/`enrollment_id` demoted to OPTIONAL context; standalone exam/certificate/external/other fees now possible); relaxes `create_fee_obligations` + the two fee triggers while keeping all context validation; makes `enrollment_id`/`course_name` nullable; records a **tracked divergence** from the registration-anchored DAT-003/004/005 model reconciled at Documentation Freeze; financial firewall absolute (obligations immutable, non-deletable, move no cash — only a receipt does); migration created in-repo, NOT applied to Production; opens/advances no phase; Owner-Decision ADR (GOV-013 Panel not invoked) | 5 | ACCEPTED | — |

Next available ADR number: **ADR-0078**.
