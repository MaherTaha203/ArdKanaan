...
| 4 | **DDL Specification** | Documented data model (`DB-NNN`): entities, attributes, keys, constraints, integrity rules — as specification documents, **not** executable SQL | `docs/data/` | ✅ **COMPLETE / CLOSED** — **Phase 4 CLOSED 2026-07-28 (ADR-0068 / AUD-P4-FINAL)**: the logical data model DAT-001…DAT-006 (DB-001…DB-159) is complete, frozen, contiguous (0 gaps/dupes/orphans), and fully traceable to frozen truth; AUD-P4-FINAL CLOSURE-READY (all A–J pass); next phase **Phase 5 — Component Library Specification** (immediate successor per GOV-011 §3; the physical database is Phase 10, reached later via Phases 5→9), pending Owner authorization. *History:* opened 2026-07-22 (ADR-0060); **P4-000** DDL Specification Master Plan adopted (LIVING v1.1.0); **DC1 COMPLETE — DAT-001 Data Model Constitution FROZEN v1.0.0** (2026-07-28, ADR-0061 / AUD-P4-001; six-kind DB-atom taxonomy, the Authority Boundary grounded in constitutional authority, technology-neutral logical representation; reviewed under GOV-013, 6/6 Panel SOUND + Judge READY); **DC2 — DAT-002 Party Entities (Student & Teacher) FROZEN v1.0.0** (2026-07-28, ADR-0063 / AUD-P4-002; DB-001…DB-021; 6/6 Panel SOUND + Judge READY), preceded by the **DR-091** person-identity amendment (ADR-0062, DOM-004 v3.10.0); **DAT-003 Programs & Registrations (+ Revenue Distribution Policy) FROZEN v1.0.0** (2026-07-28, ADR-0064 / AUD-P4-003; DB-022…DB-052; the first Phase-4 Relationship atoms; 4/4 Panel READY-WITH-NITS + Judge READY); **DAT-004 Vouchers FROZEN v1.0.0** (2026-07-28, ADR-0065 / AUD-P4-004; DB-053…DB-117; five voucher entities + the immutable split snapshot; Judge NOT-READY on two DV-1 citation orphans, repaired then frozen); **DAT-005 Derived Balances FROZEN v1.0.0** (2026-07-28, ADR-0066 / AUD-P4-005; DB-118…DB-143; the "mirror" store-nothing document — derivation basis + invariants of every derived financial quantity; Judge NOT-READY on a global-floored Teacher Payables isolation bug, corrected then frozen); **DAT-006 Activity Timeline FROZEN v1.0.0** (2026-07-28, ADR-0067 / AUD-P4-006; DB-144…DB-159; the append-only Operations event log — a stored entity that is never a second source of truth; Judge READY); documents the data model as specification (DAT-NNN docs / DB atoms), NOT executable SQL. **The entity set DAT-001…DAT-006 is COMPLETE (DB-001…DB-159); the logical data model is fully specified.** Next: formal Phase-4 closure (AUD-P4-FINAL + closure ADR), pending Owner order |
| 5 | **Component Library Specification** | Design language + component contracts — **CMP-NNN** documents / **CP-NNN** atoms; as specification documents, **no code/HTML/CSS**. Governing plan **P5-000** (LIVING v1.1.0, ADR-0069/0070); runs under **Owner-controlled design approval** (P5-000 §4) | `docs/components/` | ✅ **OPEN / IN PROGRESS** — opened 2026-07-29 (ADR-0069); GOV-011 §2 met (Phase 4 CLOSED; gates passed; Owner authorization). VEM authorized (ADR-0070); **first design decision approved — general visual direction** (ADR-0071). **CC1 COMPLETE — CMP-001 Design Language Constitution FROZEN v1.0.0** (2026-07-29, ADR-0072): the Phase-5 framework (Presentation Boundary; five-kind CP-atom taxonomy; primitive→semantic→component token architecture; component-contract architecture; cross-component grammar; desktop-first/responsively-resilient; outputs-first-class; design-decision process; CMV-01…12) — reviewed under GOV-013 (adversarial Panel + independent Judge READY; Owner-approved); **decides no visual value, authors no CP atom.** ADR-0072 also partially amends ADR-0071 (GOV-004 §5): palette descriptors (monochrome / black / blue / "Vercel-style") reclassified NON-AUTHORITATIVE; live from ADR-0071 = Sophisticated Operational Simplicity (structural direction) + LIGHT + all exact values deferred. Next: the family documents beginning at the first detailed design decision (D0), **pending a separate explicit Owner order**. *(2026-08-23 — administrative, ADR-0073: curated ECC review tooling adopted under `.claude/agents/ecc/` + `.claude/rules/ecc/` — 19 non-authoritative developer aids; opens/advances no phase, authors no CMP/CP atom, changes no frozen truth or financial logic; implementation track stays gated.)* *(2026-08-25 — product work-line, ADR-0074: Owner-directed pragmatic product track — the `app/` prototype may be evolved toward a usable product under the approved B+ UX direction, **ahead of Documentation Freeze**, as an explicit Owner-authorized exception. NON-AUTHORITATIVE: authors no CP atom / no design authority (design authority stays in the Phase-5 specs), opens/advances no phase, financial firewall absolute, reconciled to CMP/CP/SC on Documentation Freeze.)* *(2026-09-19 — product work-line, ADR-0076: Owner-authorized **student archive lifecycle** — an independent student-level `archived` state (`active`/`completed`/`archived`) with dedicated `archive_student`/`unarchive_student` RPCs, active-course eligibility, owner-only authorization, and **zero financial impact**. Records an exception to the frozen **DB-014** (DAT-002, "no student-level deactivation") for archiving only, as a **tracked divergence** reconciled at Documentation Freeze (GOV-004 §5; DAT-002 text left unedited). `completed` kept distinct; `complete_student`/`reactivate_student` unchanged. Authors no CP atom, opens/advances no phase, financial firewall absolute; Production READ-ONLY.)* *(2026-09-19 — product work-line, ADR-0077: Owner-authorized (Path B) **student-anchored financial obligations** — fee obligations become independent of course enrollment (`course_id`/`enrollment_id` demoted to OPTIONAL context; standalone exam/certificate/external/other fees and fees for non-enrolled students now possible); `create_fee_obligations` + the two fee triggers relaxed while keeping all context validation; `enrollment_id`/`course_name` nullable. Financial firewall absolute — obligations immutable, non-deletable, move no cash (only a receipt does; receipt/allocation logic unchanged). Recorded as a **tracked divergence** from the registration-anchored DAT-003/004/005 model reconciled at Documentation Freeze (GOV-004 §5; frozen DAT text left unedited). Migration created **in-repo only — NOT applied to Production**; authors no CP atom, opens/advances no phase; Production READ-ONLY.)* |
