...
| 2 | **Business Constitution** | Governed by **P2-000** (ADR-0037): framework **BC-000** + **BC-001…BC-009** — business rules (`BR-001…087`) for programs, teachers, payers, distribution policies, vouchers, operations, statements, balances; calculation rules; immutability rules for stored splits; traceability matrix (BC-009). Ownership authority: **GOV-012**. Governed by BC-000 Dual Authority (Truth + Constitutional Legitimacy) | `docs/business/` | 🔵 **IN PROGRESS (all documents FROZEN)** — P2-000 adopted (Option A); **BC-000…BC-009 all frozen; checkpoints C1…C5 COMPLETE**; 87 BR dual-cited, 0 orphan, 0 gap (BC-009 sink; BX-1…BX-6 demonstrated). **Phase 2 NOT yet declared closed** — constitutional closure and Phase-3 opening await a separate Owner authorization order (Proof ≠ Authorization) |
