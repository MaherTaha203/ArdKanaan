...
- **Modeling decisions (architect-level, grounded in frozen truth):**
  ...
  2. **Each derived Attribute is anchored to an already-frozen subject** (an Attribute must belong to an
     entity, DAT-001 §3). The three center-level balances attach to the **frozen Training-Center domain
     entity** (DOM-002 §1) — a permanent persistable singleton that DAT-002 deliberately leaves
     un-materialized as stored data (DAT-002 §4: "Owner and Training Center are singletons … not part of
     this family"). Referencing it (rather than declaring a DB Entity atom) is the only grammar-conformant
     option: an Entity atom asserts a *stored subject* (DAT-001 §3), which DAT-005's zero-storage mandate
     forbids, and the frozen doctrine forbids a **stored balance** — not the notion of a Center subject
     (DR-010: center balances are derived, never maintained by hand; DR-009 for the teacher side). The
     teacher quantities attach to the **Teacher×Program** pairing realized by the frozen Program (DB-022)
     under its permanent Program→Teacher relationship (DAT-003 DB-049/DB-032, Teacher DB-002); the
     registration readings to the frozen Registration (DB-038); Student×Program Net-Paid to the frozen
     Student (DB-001) at Student×Program scope.
...
| **DB-119** | Center | **Teacher Payables** — money currently owed to teachers = **Σ over every Teacher×Program of `max(0, that pair's Outstanding)` (DB-122)** — the aggregate of the individually-floored per-Teacher×Program balances, floored **per program before aggregation** (never a global net, so an over-paid/debt program is never offset against a payable one) | DR-015; DR-016; DR-031; DR-064; DR-066; DOM-002 §11b |
| **DB-120** | Center | **Center Net Balance** — the center's own earned share = Σ center-share snapshots (DB-074) of posted program-fee receipts **+ Σ posted non-program receipt amounts (DB-069 where revenue source DB-072 ≠ program fee, per DB-077/DB-080 — non-program income is entirely center revenue)** − center portion of posted refunds (DB-092) − Σ posted center-expense amounts (DB-084 where kind DB-086 = center expense) + Σ posted expense-return amounts (DB-101); never includes any teacher share | DR-016; DR-017; DR-042; DR-052; DR-060; DR-012; DR-081; DOM-002 §11c; DOM-002 §15a |
...
| **DB-136** | **Teacher Debt bounded & closed at zero** — a finite balance in the range **[0, original]**; never negative, and closed (settled) at zero (its non-expiry over time is the Integrity rule DB-142; the direct-repayment discharge *record* is deferred, UNK-026) | BC-004 BR-048; DR-067; BC-004 INV-19 |
...
| **DB-139** | **Posted-only derivation scope** — every sum aggregates only **Posted, non-cancelled** vouchers (DAT-004 statuses DB-056/DB-075/DB-087/DB-095/DB-103); a cancellation reverses all derived effects, restoring the pre-cancellation revelation without a new obligation | DR-045; DR-047; BC-006 BR-066 |
...
*FROZEN v1.0.0 (ADR-0066 / AUD-P4-005) — the fourth Phase-4 entity-specification document and the *mirror*
of the stored-fact documents: 26 Data Atoms (DB-118…DB-143) — 10 derived Attributes, 14 invariant
Constraints, 2 Integrity rules — specifying the derivation basis of every frozen derived financial quantity
while **storing nothing** (BC-007). Zero Entities/Relationships/Identities/stored Attributes; introduces no
new truth (DV-8). Full GOV-013 lifecycle: Discovery → Draft → Stage-3 Adversarial Self-Hardening (H2
derivation-bases CLEAN; H3 teacher-debt derivability + deferred-discharge faithful; Blocking Center-Net
non-program-revenue omission fixed, DR-018→DR-010/DR-009 corrected, Teacher-Outstanding zero-floor and
refund-reversal conservation added) → Constitutional Readiness Verification (4-lens Panel + independent
Judge; the Judge returned NOT-READY on one Major — Teacher Payables written as a global-floored net — which
was corrected to the **sum of per-Teacher×Program individually-floored balances** (DB-119 = Σ max(0,
DB-122)), honoring Teacher×Program isolation (DR-031/DR-064/DR-066), plus four Nit/Minor tightenings) →
Owner Approval. Amendments only via GOV-004 §5.*
