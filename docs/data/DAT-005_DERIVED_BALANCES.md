...
| Status | DRAFT |
| Version | 0.1.0 |
| Depends on | DAT-001 (framework); DAT-002 (Student DB-001, Teacher DB-002 — FROZEN); DAT-003 (Program DB-022, Registration DB-038, FRP DB-039 — FROZEN); DAT-004 (voucher stored facts — FROZEN); P4-000; BC-004, BC-006, BC-007 (frozen & locked); DOM-002 (§10 Account Statement, §11 The Three Balances, §12 Teacher Balance, §16 Teacher Debt); DOM-004 (DR-010/012/015/016/017/031/034/037/042/052/060/062/063/064/065/067/068/069); GOV-006/011/012/013 |
...
- **Modeling decisions (architect-level, grounded in frozen truth):**
  ...
  2. **Each derived Attribute is anchored to an already-frozen subject** (an Attribute must belong to an
     ...
     Net-Paid to the frozen Student (DB-001) at Student×Program scope.
...
- **Out of scope (frozen-truth boundaries):**
  - **Persistence of any kind** — no stored column, cached value, or hand-entered figure; aggregation is a
    mechanism of revelation, never a source of truth (BR-067/BR-070; DR-018).
...
| **DB-122** | Teacher×Program | **Teacher Outstanding (Teacher Balance)** = Total Teacher Entitlement (DB-121) − Total Teacher Settlements (Σ posted teacher-payment amounts DB-084 scoped to the program via DB-113); a single per-program subtraction, no receipt allocation | DR-034; BC-006 BR-060; BC-007 BR-068; DOM-002 §12 |
...
| **DB-125** | Registration | **Registration Collected-Total** (paid-to-date) = Σ amounts (DB-069) of that registration's posted, non-cancelled program-fee receipts (via DB-110); excludes non-program and cancelled receipts | DR-035; DAT-003 §5 |
...
| **DB-127** | Student×Program | **Student×Program Net-Paid** = Σ posted program-fee receipt amounts (DB-069) − Σ posted refund amounts (DB-092) for that Student×Program (via DB-115/DB-116); recalculated after every refund; the quantity a refund is bounded by (also the "Program Revenue" of DR-037 viewed at Student×Program scope) | DR-036; DR-037; DR-040 |
...
| **DB-129** | **Value conservation on posting** — for each posted program-fee receipt, Teacher Share (DB-073) + Center Share (DB-074) = receipt amount (DB-069) exactly; rounding creates no independent difference | DR-028; DAT-004 DB-079; BC-007 INV-35 |
...
| **DB-136** | **Teacher Debt bounded & closed at zero** — a finite balance in [0, original]; never negative, only decreases under settlement, closes at zero | BC-004 BR-048; DR-067; BC-004 INV-19 |
...
| **DB-141** | **Authority-Boundary conformance (stores nothing)** — none of these quantities may ever become persisted truth; each is a live revelation recomputed on read, never a stored/cached/hand-entered value; aggregation reveals, it never authors | DAT-001 §4; BC-007 BR-067; BC-007 BR-070; DR-018 |
...
*DRAFT (v0.1.0) — the fourth Phase-4 entity-specification document and the *mirror* of the stored-fact
...
(BC-007). Zero Entities/Relationships/Identities/stored Attributes. Introduces no new truth (DV-8). Not
frozen; not propagated. Subject to Stage 3 (Adversarial Self-Hardening), Constitutional Readiness
