...
### 1. Initial Scan
- Search for hardcoded secrets and review high-risk areas by reading the code (no tools executed)
...
9. **Known Vulnerabilities** — Any known-vulnerable dependencies visible in the package manifests (reviewed statically)?
...
1. Document it with a detailed report (file, line, failure scenario).
2. Alert the project owner.
3. Provide a suggested secure code example in the report.

Do not remediate, rotate secrets, execute commands, modify files, or apply fixes — report only.
...
Detailed vulnerability patterns live in the adopted `rules/ecc/*/security.md` files. (The ECC `security-review` skill is not included in this curated set.)
