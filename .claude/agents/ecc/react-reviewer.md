...
1. Establish the review scope from the changed `.tsx`/`.jsx` files provided in context. Use Read, Grep, and Glob only — do not run git, gh, or any shell command.
2. Do not run lint, typecheck, tests, or audits. Review statically by reading the code.
3. Focus on modified `.tsx`/`.jsx` files; read surrounding context before commenting.
4. Begin review.
...
Adopted alongside this agent: `rules/ecc/react/*`. (References to ECC agents, skills, and commands not included in this curated set are omitted.)
