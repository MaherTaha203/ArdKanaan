...
export function GlanceWorkspace() {
  ...
  const owedCourses = useMemo(() => {
    ...
    for (const item of attention) {
      const courses = studentCourseBreakdown(item.student.id, statementLines, enrollments).filter((course) => course.remaining > 0).map((course) => course.courseName)
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <div className="overflow-x-auto">
            {!loaded ? <div className="p-4"><SkeletonRows rows={5} /></div> : recent.length > 0 ? (
              <table className="w-full min-w-[560px] border-collapse text-sm">
                ...
                <tbody>{recent.map((movement) => { const isReceipt = movement.movementType === 'receipt'; return <tr key={`${movement.movementType}-${movement.id}`}><td className="figure whitespace-nowrap border-b border-border px-4 py-2.5">{formatDate(movement.voucherDate)}</td><td className="border-b border-border px-4 py-2.5"><span className={isReceipt ? 'text-gold' : 'text-clay'}>{isReceipt ? 'سند قبض' : 'سند صرف'}</span></td><td className="border-b border-border px-4 py-2.5 text-muted-foreground">{statement(movement)}</td><td className={`figure border-b border-border px-4 py-2.5 text-end font-bold ${isReceipt ? 'text-gold' : 'text-clay'}`}>{isReceipt ? '+' : '−'}{formatNumber(movement.amount)}</td><td className="border-b border-border px-4 py-2.5"><span className="inline-flex border border-gold/25 bg-gold-weak px-2.5 py-0.5 text-[11px] font-medium text-gold">مُرحَّل</span></td></tr> })}</tbody>
              </table>
        ...
        <section aria-labelledby="attention-heading" className="rounded-2xl border border-border bg-panel">
          <div className="flex items-baseline justify-between gap-4 border-b border-border px-5 py-4"><h2 id="attention-heading" className="text-base font-bold text-foreground">طلاب لديهم أرصدة مستحقة</h2><button type="button" onClick={() => navigate('students')} className="text-xs font-semibold text-olive">عرض الطلاب</button></div>
          <div className="px-2 py-1.5">
            {!loaded ? <div className="p-3"><SkeletonRows rows={3} /></div> : attention.length > 0 ? attention.map((item) => (
              <button key={item.student.id} type="button" onClick={() => selectStudent(item.student.id)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-start hover:bg-highlight">
                ...
                <span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-foreground">{item.student.name}</span><span className="block truncate text-[11px] font-medium text-warn">{owedCourses.get(item.student.id) || 'مستحق'}</span></span>
