      ...
      <ActionSheet title={isEdit ? 'تعديل سند قبض' : 'سند قبض'} onClose={closeOverlay}>
        ...
        {loadingEdit ? <p className="py-10 text-center text-sm text-faint">جارٍ تحميل السند…</p> : savedVoucher ? (
          ...
          <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <Field label="المبلغ المقبوض" error={form.formState.errors.amountReceived?.message}>{(control) => <div className="flex items-center gap-2 rounded-xl border border-olive/30 bg-olive-weak/40 px-4 py-1 focus-within:border-olive focus-within:ring-2 focus-within:ring-olive/20"><input type="number" min="1" step="1" inputMode="numeric" readOnly={isEdit} className="figure h-12 w-full bg-transparent text-2xl font-semibold text-foreground outline-none placeholder:text-faint" placeholder="0" {...control} {...form.register('amountReceived', { valueAsNumber: true })} /><span className="text-sm font-medium text-muted-foreground">₪</span></div>}</Field>
