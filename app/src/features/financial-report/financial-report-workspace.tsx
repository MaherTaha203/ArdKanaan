...
import { ArrowDownToLine, ArrowUpFromLine, Ban, Pencil, Printer, RotateCw } from 'lucide-react'
...
export function FinancialReportWorkspace() {
  ...
  return (
    <div className="space-y-4">
      ...
      <RouteHeader eyebrow="التقارير المالية" title={printTitle} actions={<><Button variant="default" onClick={() => openOverlay('receive')}><ArrowDownToLine className="size-4" />سند قبض</Button><Button variant="outline" onClick={() => openOverlay('expense')}><ArrowUpFromLine className="size-4" />سند صرف</Button><span aria-hidden className="mx-1 hidden h-7 w-px self-center bg-border sm:block" /><Button variant="quiet" onClick={() => setPrinting(true)} disabled={!loaded || viewMovements.length === 0 || view === 'external'}><Printer className="size-4" />طباعة</Button><Button variant="outline" onClick={() => void reload()} disabled={isLoading}><RotateCw className="size-4" />{isLoading ? 'جارٍ التحديث…' : 'تحديث'}</Button></>} />
