...
export function FinancialReportWorkspace() {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="border-y border-border py-4"><div className="flex flex-wrap gap-2">{PERIODS.map((item) => <button key={item.id} type="button" onClick={() => setPeriod(item.id)} aria-pressed={period === item.id} className={`rounded-full px-3 py-1.5 text-xs font-medium ${period === item.id ? 'bg-olive-weak text-olive' : 'text-muted-foreground'}`}>{item.label}</button>)}</div><div className="mt-3 grid gap-3 md:grid-cols-3"><label className="text-[12px] text-muted-foreground">الحساب<input className="mt-1 block w-full rounded-xl border border-border-strong bg-panel px-3 py-2 text-sm" value={accountName} onChange={(event) => setAccountName(event.target.value)} /></label><label className="text-[12px] text-muted-foreground">من<input type="date" className="mt-1 block w-full rounded-xl border border-border-strong bg-panel px-3 py-2 text-sm" value={fromDate} onChange={(event) => setFromDate(event.target.value)} /></label><label className="text-[12px] text-muted-foreground">إلى<input type="date" className="mt-1 block w-full rounded-xl border border-border-strong bg-panel px-3 py-2 text-sm" value={toDate} onChange={(event) => setToDate(event.target.value)} /></label></div></section>
      {view === 'external' ? (
