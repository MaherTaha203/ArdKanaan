...
export function FinancialReportWorkspace() {
  ...
  return (
    <div className="space-y-8">
      ...
      {printing ? (
        view === 'general' ? (
          <FinancialReportPrint view={view} title={title} net={genTotals.net} totalIn={genTotals.totalIn} totalOut={genTotals.totalOut} opening={genOpening} receiptCount={receiptCount(genScoped)} paymentCount={paymentCount(genScoped)} movements={genScoped} onClose={() => setPrinting(false)} />
        ) : (
      ...
      {cancelTarget ? <CancelVoucherDialog movement={cancelTarget} onClose={() => { setCancelTarget(null); setPreviewId(null); void reload() }} /> : null}
    </div>
  )
}
