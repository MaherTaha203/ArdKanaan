...
-- A UUID idempotency key is not sufficient by itself: the same key must not be
-- reusable for a different allocation payload. Store a deterministic fingerprint
-- of every financially relevant request field, including normalized allocations.
alter table public.receipt_vouchers
  add column if not exists idempotency_payload_hash text;

alter table public.receipt_vouchers
  add constraint receipt_vouchers_idempotency_payload_hash_format
  check (idempotency_payload_hash is null or idempotency_payload_hash ~ '^[0-9a-f]{32}$');
...
begin
  ...
  -- Canonicalize allocation order so semantically identical arrays have the
  -- same fingerprint while a different target/amount cannot silently replay.
  select md5((jsonb_build_object(
    ...
    'allocations', coalesce((
      select jsonb_agg(jsonb_build_object(
        'type', x->>'type',
        'enrollment_id', nullif(x->>'enrollment_id', ''),
        'fee_obligation_id', nullif(x->>'fee_obligation_id', ''),
        'amount', (x->>'amount')::numeric
      ) order by x->>'type', coalesce(x->>'enrollment_id', ''), coalesce(x->>'fee_obligation_id', ''), (x->>'amount')::numeric)
      from jsonb_array_elements(p_allocations) x
    ), '[]'::jsonb)
  ))::text) into p_payload_hash;
  ...
  if found then
    if v_existing.student_id is distinct from p_student_id
       ...
       or v_existing.voucher_date is distinct from p_date
       or v_existing.idempotency_payload_hash is distinct from p_payload_hash then
  ...
  for a in select value from jsonb_array_elements(p_allocations) loop
    ...
    if v_type = 'course' then
      ...
      if v_enrollment_id = any(v_seen_enrollments) then raise exception 'DUPLICATE_ENROLLMENT_ALLOCATION'; end if;
      ...
      if v_fee_id = any(v_seen_fees) then raise exception 'DUPLICATE_FEE_ALLOCATION'; end if;
      ...
      if v_amount > (v_fee_total - v_fee_paid) then raise exception 'FEE_ALLOCATION_EXCEEDS_REMAINING_BALANCE'; end if;
      if v_prior_external > v_fee_external then raise exception 'FEE_EXTERNAL_SHARE_ALREADY_EXCEEDED'; end if;
      ...
      if v_amount = (v_fee_total - v_fee_paid) then
        ...
        v_allocation_external := least(round(v_amount * v_fee_external / v_fee_total, 2), v_remaining_external);
      end if;
  ...
  insert into public.receipt_vouchers
    (student_id, student_name_snapshot, voucher_date, course_name, course_value,
     ...
     allocation_mode, idempotency_key, idempotency_payload_hash)
  values
  ...
  for a in select value from jsonb_array_elements(p_allocations) loop
    ...
    if v_type = 'course' then
      ...
      v_lock := hashtextextended('fee:' || v_fee_id::text, 0);
      perform pg_advisory_xact_lock(v_lock);
      select coalesce(sum(ra.external_share), 0) into v_prior_external
      from public.receipt_allocations ra
      join public.receipt_vouchers rv on rv.id = ra.receipt_voucher_id
      where ra.fee_obligation_id = v_fee_id and rv.cancelled_at is null;
