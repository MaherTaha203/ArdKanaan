...
where e.course_id is null and c.name = e.course_name;
...
do $$ begin
  ...
  if exists (
    select 1 from public.fee_obligations f
...
alter table public.fee_obligations alter column enrollment_id set not null;
...
begin
  if new.enrollment_id is null then
    ...
    if v_enrollment_id is null then raise exception 'ENROLLMENT_REQUIRED_FOR_FEE_OBLIGATION'; end if;
...
-- Reassert the atomic receipt contract using enrollment identity for every allocation.
create or replace function public.post_receipt_with_allocations(payload jsonb)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare
  p_student_id uuid; p_student_name text; p_date date; p_amount numeric;
  p_payer_name text; p_notes text; p_allocations jsonb;
  v_sum numeric := 0; v_external numeric := 0;
  v_fee_count int := 0; v_course_count int := 0;
  v_institute_fee_count int := 0; v_external_fee_count int := 0; v_shared_fee_count int := 0;
  v_receipt_id uuid; v_voucher_number bigint;
  v_summary_name text := 'تحصيل متعدّد'; v_summary_value numeric := 0;
  a jsonb; v_type text; v_amount numeric; v_enrollment_id uuid; v_fee_id uuid;
  v_course_fee numeric; v_course_paid numeric; v_fee_total numeric; v_fee_paid numeric;
  v_fee_external numeric; v_fee_category text; v_lock bigint;
begin
  ...
  for a in select value from jsonb_array_elements(p_allocations) loop
    v_type := a->>'type'; v_amount := (a->>'amount')::numeric;
    if v_amount is null or v_amount <= 0 or v_amount <> trunc(v_amount) then raise exception 'INVALID_RECEIPT_ALLOCATION'; end if;
    ...
    if v_type = 'course' then
      ...
      v_lock := hashtextextended('enrollment:' || v_enrollment_id::text, 0); perform pg_advisory_xact_lock(v_lock);
      select e.course_value into v_course_fee from public.enrollments e
      where e.id = v_enrollment_id and e.student_id = p_student_id for share;
      ...
      from public.receipt_allocations ra join public.receipt_vouchers rv on rv.id = ra.receipt_voucher_id
      ...
      if v_summary_value = 0 then select e.course_name, e.course_value into v_summary_name, v_summary_value from public.enrollments e where e.id = v_enrollment_id; end if;
    elsif v_type = 'fee' then
      v_fee_count := v_fee_count + 1; v_fee_id := nullif(a->>'fee_obligation_id', '')::uuid;
      if v_fee_id is null then raise exception 'FEE_ALLOCATION_REQUIRES_OBLIGATION'; end if;
      v_lock := hashtextextended('fee:' || v_fee_id::text, 0); perform pg_advisory_xact_lock(v_lock);
      select f.amount, f.external_share, f.fee_category into v_fee_total, v_fee_external, v_fee_category
      from public.fee_obligations f where f.id = v_fee_id and f.student_id = p_student_id and f.cancelled_at is null;
      ...
      else v_shared_fee_count := v_shared_fee_count + 1; end if;
      if v_summary_value = 0 then select f.description, f.amount into v_summary_name, v_summary_value from public.fee_obligations f where f.id = v_fee_id; end if;
    else raise exception 'INVALID_RECEIPT_ALLOCATION'; end if;
  end loop;

  if v_sum <> p_amount then raise exception 'RECEIPT_ALLOCATION_TOTAL_MISMATCH'; end if;
  if v_fee_count = 0 then v_fee_category := null; v_external := 0;
  elsif v_course_count > 0 or (v_institute_fee_count > 0 and v_external_fee_count > 0)
     or (v_external_fee_count > 0 and v_shared_fee_count > 0)
     or (v_institute_fee_count > 0 and v_shared_fee_count > 0) then v_fee_category := 'mixed';
  elsif v_external_fee_count > 0 then v_fee_category := 'external';
  elsif v_shared_fee_count > 0 then v_fee_category := 'shared';
  else v_fee_category := 'institute'; end if;
  ...
  insert into public.receipt_vouchers
    (student_id, student_name_snapshot, voucher_date, course_name, course_value, amount_received, payer_name, notes, fee_category, external_share, allocation_mode)
  values (p_student_id, p_student_name, p_date, v_summary_name, v_summary_value, p_amount, p_payer_name, p_notes, v_fee_category, v_external, true)
  ...
  for a in select value from jsonb_array_elements(p_allocations) loop
    ...
    values (v_receipt_id, a->>'type', nullif(a->>'enrollment_id', '')::uuid, nullif(a->>'fee_obligation_id', '')::uuid, (a->>'amount')::numeric);
  end loop;
